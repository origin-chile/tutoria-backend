...
async def chat(req: ChatRequest):
    ...
    # Verificar límite
    remaining = None
    token_to_check = req.access_token or "free-trial"
    # Crear entrada temporal para free-trial si no existe
    if token_to_check == "free-trial":
        tokens = load_tokens()
        if "free-trial" not in tokens:
            expires = (datetime.now() + timedelta(days=365)).isoformat()
            tokens["free-trial"] = {"plan": "semanal", "expires": expires, "flow_token": None}
            save_tokens(tokens)
    allowed, remaining = check_and_increment(token_to_check)
    if not allowed:
